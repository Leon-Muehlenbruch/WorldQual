#Version von RiverSection.R mit 2 inputdateien. aus zwei runs:
# aus ersten Input alles nehmen wie gehabt, 
# aus den zweiten nur calculated concentration (1.Diagramm) monthly loading accumulated (3.Diagramm)
...
#### zweite Datei
x2<-file_wq_stat(filename2)

# nur wenn Datensatz nicht leer ist und Daten sollen gleiche Flussabschnitt und Zeitspanne haben, damit auch Anzahl der Zeilen
if(dim(x[[2]])[1]>0 && dim(x[[2]])[1]==dim(x2[[2]])[1]) { 
...
#for (month in c(200001, 200002, 200003))
{
  ...
  if (substr(RiverSection.diag,1,1) != 0) {
    if (counter==1) {
      ...
      open_file(paste(filename_begin, head_data$river_name,"_",head_data$IDrun,"_",head_data2$IDrun,"_",counter_files,".png", sep="") 
    ...
    y_matrix2=subset(stat_data2[, c(3, 6)], date == month)[, 2]   # y-Achse
    y_matrix$concentration2<-y_matrix2
    y_matrix[,6][y_matrix[,6]==0] <- NA    # concentration2
    rm(y_matrix2)
    ...
    if(min_axis_y>0) {
      ...
      y_matrix[,6][y_matrix[,6]<min_axis_y] <- NA  # calculated concentration2
    }
    ...
    if(max_axis_y>0) {
      ...
      y_matrix[,6][y_matrix[,6]>max_axis_y] <- NA  # calculated concentration2
    }
    ...
    # maximal 6 Reihen in der Diagramm: 
    # sicher, sicher_measured, unsicher, unsicher_measured, calculated, calculated2
    # measured könnten nicht alle da sein, dann sie nicht abbilden
    diagramme_rows<-c(0,0,0,0,1,1)
    ...
    color_matrix= c("red", "red", "brown4", "brown4", "blue", "springgreen")[idx.reihen]
    ...
    lty_matrix = c("blank", "blank", "blank", "blank", "solid", "twodash")[idx.reihen]
    lwd_matrix=c(8, 8, 8, 8, 1, 1)[idx.reihen]; pch_matrix = c(15, 15, 15, 15, NA, NA)[idx.reihen]
    type_col = c("p","p","p","p","l","l")[idx.reihen]
    legend_matrix = c("measured", "measured", "measured uncert", "measured uncert"
                      , paste("calculated",input.name,sep="")
                      , paste("calculated",input.name2,sep=""))[idx.reihen]
    y.columns <- c(7, 9, 8, 10, 5, 6)[idx.reihen] #c(4,5,3)
  ...
  if (substr(RiverSection.diag,3,3) != 0) {
    ...
    y_matrix[,1][y_matrix[,1]==0] <- NA # calculated loading
    y_matrix[,2][y_matrix[,2]==0] <- NA # measured loading
    y_matrix2=subset(stat_data2[, c(3,19)], date == month)[, 2]    # y-Achse
    y_matrix$`monthly loading accumulated2`<-y_matrix2
    y_matrix[,4][y_matrix[,4]==0] <- NA # calculated loading2
    ...
    if(min_axis_y>0) {
      ...
      y_matrix[,4][y_matrix[,4]<min_axis_y] <- NA    # monthly grid cell loading accumulated2
    }
    ...
    if(max_axis_y>0) {
      ...
      y_matrix[,4][y_matrix[,4]>max_axis_y] <- NA    # monthly grid cell loading accumulated2
    }
    ...
    if (length(which(is.na(y_matrix$unsicher))) != length(y_matrix$unsicher)) {
      unsicher_gibts = TRUE
      color_matrix= c("darkolivegreen3", "salmon", "brown4", "red")
      lty_matrix = c("solid", "solid", "blank", "blank"); lwd_matrix=c(1, 1, 8, 8); pch_matrix = c(NA, NA, 17, 17)
      type_col = c("l", "l", "p", "p")
      legend_matrix = c("monthly loading accumulated", "monthly loading accumulated2"
                        , "measured monthly loading", "measured monthly loading uncert")
      y.columns <- c(1,4,5,6)
    } else {
      unsicher_gibts = FALSE
      color_matrix= c("darkolivegreen3", "salmon", "red")
      lty_matrix = c("solid", "solid", "blank"); lwd_matrix=c(1, 1, 8); pch_matrix = c(NA, NA, 17)
      type_col = c("l", "l", "p")
      legend_matrix = c(paste("monthly loading accumulated",input.name,sep="")
                        , paste("monthly loading accumulated",input.name2,sep="")
                        , "measured monthly loading")
      y.columns <- c(1,4,5)
    }
